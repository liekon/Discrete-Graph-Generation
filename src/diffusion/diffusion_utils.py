...
def sample_discrete_features(limit_dist, probX, probE, node_mask):
    ...
    probX[~node_mask] = limit_dist.X[None, None, :].type_as(probX) #1 / probX.shape[-1] #torch.tensor([0.7230, 0.1151, 0.1593, 0.0026]).to(probX.device) #
    ...
    probE[inverse_edge_mask] = limit_dist.E[None, None, None, :].type_as(probE) #1 / probE.shape[-1] #torch.tensor([0.7261, 0.2384, 0.0274, 0.0081, 0.0]).to(probE.device)#
    probE[diag_mask.bool()] = limit_dist.E[None, None, None, :].type_as(probE) #1 / probE.shape[-1] #torch.tensor([0.7261, 0.2384, 0.0274, 0.0081, 0.0]).to(probE.device)#
